  ...
  const filteredEdges = useMemo(() => {
    ...
    out = [...out].sort((a, b) => {
      ...
      switch (filter.sortKey) {
        case 'name':
          return na.name.localeCompare(nb.name) * dir;
        case 'runs':
          return ((na.stats?.totalTestRuns ?? 0) - (nb.stats?.totalTestRuns ?? 0)) * dir;
        case 'rate':
          return ((na.stats?.successRate ?? 0) - (nb.stats?.successRate ?? 0)) * dir;
        case 'last': {
          const ta = na.stats?.lastRunTime ? new Date(na.stats.lastRunTime).getTime() : 0;
          const tb = nb.stats?.lastRunTime ? new Date(nb.stats.lastRunTime).getTime() : 0;
          return (ta - tb) * dir;
        }
        default:
          return 0;
    ...
    <div className="space-y-6">
      ...
      {edges.length === 0 ? (
        <EmptyState
          ...
          description="Create one above, or seed sample data with `make docker-test-seed`."
          action={
            <Button onClick={() => setCreating(true)}>
