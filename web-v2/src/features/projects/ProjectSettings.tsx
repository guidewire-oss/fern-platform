...
type TabId = 'general' | 'integrations' | 'team' | 'notifications';
...
function readTabFromHash(): TabId {
  ...
  if (h === 'general' || h === 'integrations' || h === 'team' || h === 'notifications') {
    return h;
  }
  return 'general';
}
      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-2 border-t border-border pt-3">
            {update.error && (
              <span className="text-xs text-red-700">{(update.error as Error).message}</span>
            )}
...
function IntegrationsTab({ projectId }: { projectId: string }) {
  ...
  // explicitly so it doesn't try to read from a different route.
  return <JiraConnectionsContent projectId={projectId} />;
}
