  ...
  return (
    <header className="space-y-2">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Stat label="Passed"  value={run.passedTests}  tone="text-green-700" />
        <Stat label="Failed"  value={run.failedTests}  tone="text-red-700" />
        <Stat label="Skipped" value={run.skippedTests} tone="text-amber-700" />
      </div>
            ...
            {suites.map((s) => (
              ...
              >
                ...
                <TD className="text-right tabular-nums text-sm">
                  <span className="text-green-700" title="passed">{s.passedSpecs}</span>
                  {' / '}
                  <span className="text-red-700"   title="failed">{s.failedSpecs}</span>
                  {' / '}
    ...
    <>
      ...
      {open && (spec.errorMessage || spec.stackTrace) && (
        <TR>
          <TD colSpan={6}>
            <pre
              className="overflow-x-auto rounded bg-slate-50 p-2 font-mono text-[11px] text-slate-700"
  ...
  return (
    <Card>
      ...
      {open && (
        <CardBody>
          <pre
            className="overflow-x-auto rounded bg-slate-50 p-2 font-mono text-xs text-slate-700"
