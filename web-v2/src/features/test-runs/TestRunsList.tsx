                  ...
                  {data?.edges.map(({ node }) => (
                    <TR key={node.id}>
                      ...
                      <TD className="text-right tabular-nums text-sm">
                        <span className="text-green-700" title="passed">
                          {node.passed_tests}
                        </span>
                        {' / '}
                        <span className="text-red-700" title="failed">
                          {node.failed_tests}
                        </span>
                        {' / '}
...
function SavedViewsBar({
  ...
  const views = useTestRunsSavedViews();
          ...
          >
            ...
            {!views.isLoading && list.length === 0 && (
              <div className="px-3 py-2 text-xs text-muted">
                You haven't saved any views yet. Configure filters below and
                click "Save view".
              </div>
            )}
            {list.map((v) => (
              <div
                key={v.id}
                className="flex items-center justify-between gap-2 px-3 py-1.5 hover:bg-surface-2"
              >
                <button
