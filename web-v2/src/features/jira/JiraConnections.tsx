...
import { ArrowLeft, Plug, Plus, Trash2, Zap, Pencil } from 'lucide-react';
...
import {
  ...
  useUpdateJiraConnection,
  useUpdateJiraCredentials,
  type JiraConnection,
...
function EditModal({
  ...
  const [authenticationType, setAuthType] = useState<NewJiraConnection['authenticationType']>('api_token');
  const [username, setUsername] = useState('');
  const [credential, setCredential] = useState('');

  const update = useUpdateJiraConnection(projectId);
  const updateCreds = useUpdateJiraCredentials(projectId);

  // Re-seed the form each time a different connection is opened.
  useEffect(() => {
    if (!connection) return;
    setName(connection.name);
    setJiraUrl(connection.jiraUrl);
    setProjectKey(connection.projectKey);
    setVersionFilter(connection.versionFilter ?? '');
    setAuthType(connection.authenticationType);
    setUsername(connection.username);
    setCredential('');
  }, [connection]);

  const valid = name.trim() && jiraUrl.trim() && projectKey.trim() && username.trim();
  const pending = update.isPending || updateCreds.isPending;
  const err = (update.error ?? updateCreds.error) as Error | null;

  const save = async () => {
    if (!connection) return;
    await update.mutateAsync({
      id: connection.id,
      input: { name: name.trim(), jiraUrl: jiraUrl.trim(), projectKey: projectKey.trim(), versionFilter: versionFilter.trim() },
    });
    // Only rotate the credential when a new one is entered — the endpoint
    // requires a value, and the secret is otherwise left untouched.
    if (credential.trim()) {
      await updateCreds.mutateAsync({
        id: connection.id,
        input: { authenticationType, username: username.trim(), credential: credential.trim() },
      });
    }
    onClose();
  };
  ...
  return (
    <Modal
      ...
      description="Leave the token blank to keep the current credential."
      size="lg"
      footer={
        <>
          <Button variant="secondary" onClick={onClose} disabled={pending}>Cancel</Button>
          <Button disabled={!valid || pending} onClick={save}>
            {pending ? <Spinner className="text-white" /> : 'Save changes'}
          </Button>
        </>
      }
    >
      <div className="space-y-3">
        <Row>
          ...
          <Field label="Project key" required hint="JIRA project key (e.g. FERN).">
            <Input value={projectKey} onChange={setProjectKey} />
          </Field>
        </Row>
        <Field label="JIRA URL" required>
          <Input type="url" value={jiraUrl} onChange={setJiraUrl} />
        </Field>
        <Row>
          <Field label="Auth method" required>
            <select
              value={authenticationType}
              onChange={(e) => setAuthType(e.target.value as NewJiraConnection['authenticationType'])}
              className="w-full rounded border border-border bg-surface px-2.5 py-1.5 text-sm focus:border-primary focus:outline-none"
            >
              <option value="api_token">API token</option>
              <option value="personal_access_token">Personal access token</option>
        ...
        <Field label="New token / credential" hint="Leave blank to keep the current credential; enter a value to rotate it.">
          <Input type="password" value={credential} onChange={setCredential} placeholder="••••••••" />
        </Field>
        <Field label="Version filter" hint="Comma-separated release prefixes for the coverage picker (blank = all).">
          <Input value={versionFilter} onChange={setVersionFilter} placeholder="1.,2." />
        </Field>
        {err && (
          <div className="rounded border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs text-red-800">
            {err.message}
